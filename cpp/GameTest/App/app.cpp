...
{	
	...
	void DrawCircle(float x, float y, float ra, float r, float g, float b)
	{
		float delta = 0.00001;
		glBegin(GL_LINE_LOOP);
		glColor3f(r, g, b);

		for (float i = 0; i <= PI * 2; i += delta)
		{
			float cx = x + ra * cos(i); // calculate the coordinate on x axis 
			float cy = y + ra * sin(i); // calculate the coordinate on y axis
#if APP_USE_VIRTUAL_RES		
			APP_VIRTUAL_TO_NATIVE_COORDS(cx, cy);
#endif
			glVertex2f(cx, cy); // print line 
		}
