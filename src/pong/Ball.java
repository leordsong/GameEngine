...
public class Ball extends GameObject {
	...
	public void update(float elapsedTime) {
		...
		if (getTransform().getPosition().getX() < 0) getTransform().setPosition(new Vector2(0, getTransform().getPosition().getY()));
		else if (getTransform().getPosition().getX() > Game.WINDOW_WIDTH) getTransform().setPosition(new Vector2(Game.WINDOW_WIDTH, getTransform().getPosition().getY()));
		if (getTransform().getPosition().getY() < 0) getTransform().setPosition(new Vector2(getTransform().getPosition().getX(), 0));
		else if (getTransform().getPosition().getY() > Game.WINDOW_HEIGHT) getTransform().setPosition(new Vector2(getTransform().getPosition().getX(), Game.WINDOW_HEIGHT));
	}
